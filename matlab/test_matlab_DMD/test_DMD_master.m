...
% alp_returnvalue = calllib('alp4395','AlpSeqControl',...
%     deviceid, seqid, int32(2110), int32(2)) % ALP_DATA_FORMAT -> binary topdown
alp_returnvalue = calllib('alp4395','AlpSeqControl',...
    deviceid, seqid, int32(2104), int32(2105)); % % ALP_BIN_MODE -> normal BIN mode

%% AlpSeqTiming
illumtime_us = int32(500*pictime); %in us. minimum accepted is 10us Ignored in uninterrupted binary mode
pictime_us = int32(1000*pictime); %in us, by default is 61us in binary mode
synchdelay = int32(50); %note: micromirrors have a ~40us mechanical relax time
...
alp_returnvalue = calllib('alp4395','AlpProjControl',...
    deviceid, alp_proj_mode, int32(2301)); % ALP_MASTER

%stepping transition in MASTER MODE (level trigger)
% ALP_PROJ_STEP = int32(2329);
%  alp_returnvalue = calllib('alp4395','AlpProjControl',...
%      deviceid, ALP_PROJ_STEP, int32(2006)); % ALP_LEVEL_HIGH



%% AlpSeqPut.. upload the sequence of images
arrayptr = libpointer('voidPtr', 255*uint8(array));
picoffset = int32(0);
